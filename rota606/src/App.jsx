...
async function nGet(path, params = {}) {
  ...
  const r = await fetch(`/api/notion/${path}${qs ? '?' + qs : ''}`)
  return r.json()
}
async function nPost(path, body) {
  const r = await fetch(`/api/notion/${path}`, { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body) })
  return r.json()
}
async function nDelete(path) {
  return fetch(`/api/notion/${path}`, { method:'DELETE' })
}
...
const S = {
  app:{ minHeight:'100vh', background:'#0D1117', color:'#E6EDF3', fontFamily:"'Segoe UI',system-ui,sans-serif", fontSize:'14px' },
  header:{ background:'#161B22', borderBottom:'1px solid #21262D', padding:'0 24px', display:'flex', alignItems:'center', justifyContent:'space-between', height:'56px', position:'sticky', top:0, zIndex:100 },
  logo:{ display:'flex', alignItems:'center', gap:'10px', fontWeight:700, fontSize:'16px', color:'#10D9A0' },
  syncBar:{ display:'flex', alignItems:'center', gap:'8px', fontSize:'12px', color:'#8B949E' },
  syncDot:(ok)=>({ width:'8px', height:'8px', borderRadius:'50%', background:ok?'#10D9A0':'#F85149', flexShrink:0 }),
  syncBtn:{ background:'none', border:'1px solid #30363D', color:'#8B949E', borderRadius:'6px', padding:'4px 10px', cursor:'pointer', fontSize:'12px' },
  nav:{ display:'flex', borderBottom:'1px solid #21262D', background:'#161B22', padding:'0 24px', gap:'4px', overflowX:'auto' },
  navBtn:(a)=>({ padding:'12px 16px', background:'none', border:'none', color:a?'#10D9A0':'#8B949E', borderBottom:a?'2px solid #10D9A0':'2px solid transparent', cursor:'pointer', fontWeight:a?600:400, fontSize:'14px', whiteSpace:'nowrap' }),
  main:{ padding:'24px', maxWidth:'1100px', margin:'0 auto' },
  card:{ background:'#161B22', border:'1px solid #21262D', borderRadius:'10px', padding:'20px', marginBottom:'16px' },
  cardTitle:{ fontWeight:600, fontSize:'15px', marginBottom:'12px', color:'#E6EDF3', display:'flex', alignItems:'center', gap:'8px' },
  table:{ width:'100%', borderCollapse:'collapse', fontSize:'13px' },
  th:{ textAlign:'left', padding:'8px 12px', background:'#0D1117', color:'#8B949E', fontSize:'12px', fontWeight:600, borderBottom:'1px solid #21262D' },
  td:{ padding:'8px 12px', borderBottom:'1px solid #21262D', color:'#E6EDF3', verticalAlign:'top' },
  input:{ background:'#0D1117', border:'1px solid #30363D', borderRadius:'6px', color:'#E6EDF3', padding:'8px 12px', fontSize:'14px', width:'100%', boxSizing:'border-box', outline:'none' },
  btn:{ background:'#10D9A0', color:'#0D1117', border:'none', borderRadius:'6px', padding:'8px 16px', fontWeight:600, cursor:'pointer', fontSize:'14px' },
  btnDanger:{ background:'transparent', color:'#F85149', border:'1px solid #F8514944', borderRadius:'6px', padding:'4px 10px', cursor:'pointer', fontSize:'12px' },
  label:{ color:'#8B949E', fontSize:'12px', marginBottom:'4px', display:'block' },
  grid2:{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'16px' },
  tag:{ background:'#21262D', borderRadius:'4px', padding:'2px 8px', fontSize:'12px', color:'#8B949E', display:'inline-block', margin:'2px' },
  notionLink:{ color:'#10D9A0', fontSize:'12px', textDecoration:'none', marginTop:'10px', display:'inline-block' },
}
...
const FRESCOS_CONFIG = [
  { pdv:'809404', cliente:'Famolde P1', doces:'21,22,24,25,27', salgados:'41–47', tentacao:'48', gourmet:'—' },
  { pdv:'809449', cliente:'BA Vidros Entrada', doces:'—', salgados:'41–46', tentacao:'—', gourmet:'—' },
  { pdv:'806066', cliente:'BA Vidros Entrada (FAS 1050)', doces:'11,13,17,19,51–56', salgados:'40–49', tentacao:'15 (fixo: Hambúrguer)', gourmet:'—' },
  { pdv:'812420', cliente:'Innovcoating', doces:'mix', salgados:'—', tentacao:'A13,A14', gourmet:'—' },
  { pdv:'813878', cliente:'IMV', doces:'A13,A14', salgados:'A15–A18', tentacao:'—', gourmet:'—' },
  { pdv:'815691', cliente:'PES', doces:'21,22,24,25,28,41–48', salgados:'—', tentacao:'—', gourmet:'—' },
  { pdv:'813267', cliente:'Promoplas', doces:'A22,A24,A26,A41–A46', salgados:'—', tentacao:'—', gourmet:'—' },
  { pdv:'815504', cliente:'HRV Administração', doces:'A31,A32', salgados:'A33–A37', tentacao:'—', gourmet:'A38' },
  { pdv:'807540', cliente:'BA MG2', doces:'21,22', salgados:'41,42,43', tentacao:'44,45,46', gourmet:'—' },
]
...
const PRODUTOS = [
  ...
  {cod:'383719',nome:'Palmier chocolate',total:91},{cod:'383668',nome:'Sande pasta frango & alho',total:80},
  {cod:'383590',nome:'Sande pasta atum & cenoura',total:82},{cod:'383727',nome:'Pão com chouriço',total:66},
  {cod:'383714',nome:'Bolo de cenoura',total:60},{cod:'383711',nome:'Croissant de ovo',total:58},
  {cod:'383713',nome:'Bolo brigadeiro',total:54},{cod:'383729',nome:'Supremo misto com salsicha',total:62},
  ...
  {cod:'383724',nome:'Torta de chocolate',total:36},{cod:'383722',nome:'Bom bocado',total:42},
...
// ─── PDV CARD ───────────────────────────────────────────────
function PdvCard({ titulo, pageId, conteudo, onSave }) {
  const [texto, setTexto] = useState(conteudo || '')
  const [saving, setSaving] = useState(false)
  const [saved, setSaved] = useState(false)

  useEffect(() => { setTexto(conteudo || '') }, [conteudo])

  const handleSave = async () => {
    setSaving(true)
    await onSave(pageId, texto)
    setSaving(false)
    setSaved(true)
    ...
    <div style={S.card}>
      ...
      <div style={{ display:'flex', gap:'8px', marginTop:'10px', alignItems:'center' }}>
        <button style={S.btn} onClick={handleSave} disabled={saving}>
          {saving ? 'A guardar…' : saved ? '✅ Guardado!' : '💾 Guardar no Notion'}
        </button>
        <button style={S.btnDanger} onClick={handleClear} disabled={saving}>🗑 Limpar</button>
      </div>
      <a href={`https://notion.so/${pageId}`} target="_blank" rel="noreferrer" style={S.notionLink}>
        Abrir no Notion ↗
      </a>
    </div>
  )
    ...
    <div>
      <div style={{ ...S.card, borderColor:'#10D9A044' }}>
        ...
        <div style={{ color:'#8B949E', fontSize:'13px' }}>
          {rotaHoje.length} clientes · {rotaHoje.reduce((a,c) => a + c.maquinas.length, 0)} máquinas
        </div>
      </div>
      <div style={S.grid2}>
        <div style={S.card}>
          <div style={S.cardTitle}>📦 806477 — BA Vidros Young</div>
          ...
          <a href={`https://notion.so/${PAGE_IDS.pdv806477}`} target="_blank" rel="noreferrer" style={S.notionLink}>Abrir no Notion ↗</a>
        </div>
        <div style={S.card}>
          <div style={S.cardTitle}>📦 807542 — BA Vidros 1050</div>
          {loading ? <span style={{ color:'#8B949E' }}>A carregar…</span> :
            <pre style={{ margin:0, color:'#E6EDF3', fontSize:'13px', whiteSpace:'pre-wrap', fontFamily:'inherit' }}>
              {pdv1050 || <span style={{ color:'#8B949E' }}>Sem reposição pendente</span>}
            </pre>
          }
          <a href={`https://notion.so/${PAGE_IDS.pdv807542}`} target="_blank" rel="noreferrer" style={S.notionLink}>Abrir no Notion ↗</a>
        </div>
      </div>
    </div>
  )
    ...
    <div style={S.card}>
      ...
      {loading ? <span style={{ color:'#8B949E' }}>A carregar…</span> :
        chamados.length === 0 ? <span style={{ color:'#8B949E' }}>Nenhum chamado registado</span> : (
          <table style={S.table}>
            <thead><tr><th style={S.th}>Título</th><th style={S.th}>Link</th></tr></thead>
            <tbody>
              {chamados.map((c,i) => (
                <tr key={i}>
                  <td style={S.td}>{c.titulo}</td>
                  <td style={S.td}><a href={c.url} target="_blank" rel="noreferrer" style={{ color:'#10D9A0', textDecoration:'none' }}>Abrir ↗</a></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <a href={`https://notion.so/${PAGE_IDS.chamadosJun}`} target="_blank" rel="noreferrer" style={S.notionLink}>Ver todos no Notion ↗</a>
    </div>
  )
}
...
function TabPDVs({ pdvYoung, pdv1050, loading, onSave }) {
  return (
    <div>
      <PdvCard titulo="PDV 806477 — BA Vidros Refeitório Young" pageId={PAGE_IDS.pdv806477} conteudo={pdvYoung} onSave={onSave} />
      <PdvCard titulo="PDV 807542 — BA Vidros Refeitório 1050" pageId={PAGE_IDS.pdv807542} conteudo={pdv1050} onSave={onSave} />
    </div>
  )
}
    ...
    <div style={S.card}>
      ...
      <div style={{ overflowX:'auto' }}>
        <table style={S.table}>
          <thead>
            <tr>
              <th style={S.th}>PDV</th><th style={S.th}>Cliente</th>
              <th style={S.th}>Doces</th><th style={S.th}>Salgados</th>
              <th style={S.th}>🟡 Tentação</th><th style={S.th}>⭐ Gourmet</th>
            </tr>
          </thead>
          <tbody>
      ...
      <a href={`https://notion.so/${PAGE_IDS.frescos}`} target="_blank" rel="noreferrer" style={S.notionLink}>Ver no Notion ↗</a>
    </div>
  )
}

function TabProdutos() {
  const max = Math.max(...PRODUTOS.map(p => p.total))
  return (
    <div style={S.card}>
      ...
      <div style={{ overflowX:'auto' }}>
        <table style={S.table}>
          <thead>
            <tr>
              <th style={S.th}>Código</th><th style={S.th}>Produto</th>
              <th style={{ ...S.th, textAlign:'right' }}>Total</th><th style={S.th}>Volume</th>
            </tr>
          </thead>
          <tbody>
            {PRODUTOS.map((p,i) => (
              <tr key={i}>
                ...
                <td style={{ ...S.td, minWidth:'120px' }}>
                  <div style={{ background:'#21262D', borderRadius:'4px', height:'8px', overflow:'hidden' }}>
                    <div style={{ background:'#10D9A0', height:'100%', width:`${(p.total/max)*100}%` }} />
                  </div>
                </td>
              </tr>
            ))}
      ...
      <div style={{ display:'flex', gap:'8px', marginBottom:'16px', flexWrap:'wrap' }}>
        {dias.map(d => (
          <button key={d} onClick={() => setAtivo(d)} style={{
            background:ativo===d?'#10D9A0':'#21262D', color:ativo===d?'#0D1117':'#E6EDF3',
            border:'none', borderRadius:'6px', padding:'8px 16px', cursor:'pointer', fontWeight:600, fontSize:'13px',
          }}>{d}</button>
        ))}
      </div>
      <div style={S.card}>
...
export default function App() {
  ...
  const fetchChamados = async () => {
    const data = await nGet(`blocks/${PAGE_IDS.chamadosJun}/children`, { page_size:'50' })
  ...
  const savePdv = async (pageId, texto) => {
    const existing = await nGet(`blocks/${pageId}/children`, { page_size:'50' })
    if (existing.results) {
      await Promise.all(existing.results.map(b => nDelete(`blocks/${b.id}`)))
    }
    if (texto.trim()) {
      const lines = texto.split('\n').filter(Boolean)
      await nPost(`blocks/${pageId}/children`, {
    ...
    if (pageId === PAGE_IDS.pdv806477) setPdvYoung(texto)
    else setPdv1050(texto)
  }
